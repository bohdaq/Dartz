...
$defProp(Object.prototype, "get$2", function($0, $1) {
  return this.noSuchMethod("get", [$0, $1]);
});
...
function getLanguageData(languageName, onSuccess) {
  var url = "http://search.twitter.com/search.json?q=%23darthack";
  var request = _XMLHttpRequestFactoryProvider.XMLHttpRequest$factory();
  request.setRequestHeader("Content-Type", "application/json");
  request.setRequestHeader("Access-Control-Request-Headers", "http://127.0.0.1:3030");
  request.get$2(url, onSuccess);
}
...
function main() {
  ...
  get$$document().get$on().get$keyDown().add($wrap_call$1(function rotateHandler(event) {
    var xAngle = (0), yAngle = (0);
    switch (event.get$keyCode()) {
      case (37):

        yAngle = $sub$(yAngle, (90));
        break;

      case (38):

        xAngle = $add$(xAngle, (90));
        break;

      case (39):

        yAngle = $add$(yAngle, (90));
        break;

      case (40):

        xAngle = $sub$(xAngle, (90));
        break;

    }
    ;
    get$$document().query("#cube").style.setProperty("-webkit-transform", $add$($add$($add$($add$("rotateX(", xAngle), "deg) rotateY("), yAngle), "deg)"));
  }
  ), false);
}
(function(){
